...
function validateForm() {
    ...
    const isValid = /^T[a-zA-Z0-9]{33}$/.test(address) && amountStr && amount > 0 && amount <= currentBalance;
...
function updateBalanceDisplay() {
    ...
    if (balanceUsd) {
        const usd = (currentBalance * 0.12).toFixed(3);
...
async function handleWithdraw() {
    try {
        ...
        if (!/^T[a-zA-Z0-9]{33}$/.test(address)) return alert('Неверный TRX адрес');
