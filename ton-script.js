...
function initBalanceToggle() {
    ...
    const detailedBalance = document.querySelector('.ton-balance');
    ...
    if (eyeIcon && eyeIconDark && balanceAmount) {
        ...
        const handleEyeClick = function() {
            ...
            if (isHidden) {
                ...
                detailedBalance.textContent = '••• TON';
                toggleEyeIcons(true);
...
// ==================== ЗАГРУЗКА TON БАЛАНСА ====================

async function loadTonBalance() {
    try {
        console.log('💰 ЗАГРУЖАЕМ TON БАЛАНС...');
        ...
        console.log('📊 ПОЛУЧЕН TON БАЛАНС:', data);
        
        if (data.success && data.balance) {
            updateTonDisplay(data.balance);
        } else {
            console.log('⚠️ TON баланс не найден');
            setDefaultTonDisplay();
        }
        
    } catch (error) {
        console.error('💥 ОШИБКА ЗАГРУЗКИ TON:', error);
        setDefaultTonDisplay();
    }
}

// ОБНОВИТЬ ОТОБРАЖЕНИЕ TON
function updateTonDisplay(balance) {
    const balanceElement = document.getElementById('balanceAmount');
    const tonElement = document.getElementById('tonBalance');
    
    if (balance.ton_amount !== undefined && balance.ton_price !== undefined) {
        // USD стоимость
        const usdValue = balance.ton_amount * balance.ton_price;
        if (balanceElement) {
        ...
        // Количество TON
        if (tonElement) {
            tonElement.textContent = `${balance.ton_amount.toFixed(6)} TON`;
            console.log(`✅ КОЛИЧЕСТВО TON: ${balance.ton_amount.toFixed(6)}`);
        }
...
function updateOriginalBalances() {
    ...
    if (balanceElement && tonElement) {
        // Обновляем глобальные переменные
        window.currentOriginalBalance = balanceElement.textContent;
        window.currentOriginalDetailedBalance = tonElement.textContent;
...
function setDefaultTonDisplay() {
    ...
    if (tonElement) {
        tonElement.textContent = '0.000000 TON';
    }
    
    console.log('🔧 УСТАНОВЛЕН ДЕФОЛТ TON');
}

// ЗАГРУЗКА TON ТРАНЗАКЦИЙ
async function loadTonTransactions() {
    console.log('📊 Начинаем загрузку TON транзакций...');
    ...
    try {
        ...
        if (data.success && data.transactions) {
            ...
            if (tonTransactions.length > 0) {
                displayTransactions(tonTransactions);
            } else {
                showNoTransactions();
            }
        } else {
        ...
        console.error('📊 Ошибка загрузки TON транзакций:', error);
    ...
    div.innerHTML = `
        ...
        <div class="transaction-info">
            ...
            <div class="transaction-details">
                <span class="transaction-network">TON</span>
                <span>${dateStr}</span>
            </div>
        </div>
        <div class="transaction-amount">
            <div class="transaction-crypto ${amountClass}">
                ${amountSign}${amount.toFixed(6)} TON
            </div>
