...
let currentPriceUsd = 0; // динамическая цена SOL в USD
...
async function loadUserBalance() {
    try {
        ...
        currentPriceUsd = data?.balance?.sol_price ?? 0;
        if (!currentPriceUsd) {
            try {
                const p = await fetch('/api/prices').then(r=>r.json());
                if (p?.success) currentPriceUsd = Number(p.prices?.sol||0) || 0;
            } catch {}
        }
...
async function handleWithdraw() {
    try {
        ...
        if (response.ok && result.success) {
            ...
            alert(result.message || `Транзакция на вывод ${total.toFixed(8)} SOL (вкл. комиссию сети) создана!`);
