...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Автоматически загружаем SOL адрес
    loadSolAddress();
...
// Функция для загрузки SOL адреса
async function loadSolAddress() {
    try {
        ...
        // Получаем адрес SOL пользователя
        let networkInfo = window.userManager.getNetworkWithAddress('sol');
        
        // Если адрес пользователя не найден, используем адрес по умолчанию
        if (!networkInfo || !networkInfo.address) {
            console.warn('Адрес пользователя для SOL не найден, используем адрес по умолчанию');
            const defaultNetworkInfo = getAddress('sol');
        ...
        if (networkInfo) {
            ...
            showNetworkError('SOL');
        }
    } catch (error) {
        console.error('Ошибка загрузки SOL адреса:', error);
        showNetworkError('SOL');
    }
}
    ...
    try {
        ...
        qrContainer.innerHTML = `<div style="width: 200px; height: 200px; background-color: #f0f0f0; display: flex; align-items: center; justify-content: center; border-radius: 8px;">
            <div style="text-align: center; color: #666;">
                ...
                <div style="font-size: 10px; margin-top: 4px;">SOL</div>
            </div>
        </div>`;
    }
}
...
function shareAddress() {
    ...
    if (address) {
        const shareData = {
            title: 'SOL Адрес',
            text: 'Адрес для пополнения Solana',
            url: address
        };
