...
function updateFeeInfo() {
    ...
    document.getElementById('networkFee').textContent = '0.01 TON';
    document.getElementById('totalAmount').textContent = `${amount.toFixed(8)} TON`;
}
...
function validateForm() {
    ...
    const isValid = /^EQ[a-zA-Z0-9]{48}$/.test(address) && amountStr && amount > 0 && amount <= currentBalance;
...
async function handleWithdraw() {
    try {
        ...
        if (!/^EQ[a-zA-Z0-9]{48}$/.test(address)) return alert('Неверный TON адрес');
