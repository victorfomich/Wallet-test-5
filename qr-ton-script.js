...
// Функция для загрузки TON адреса
async function loadTonAddress() {
    try {
        ...
        // Получаем адрес TON пользователя
        let networkInfo = window.userManager.getNetworkWithAddress('ton');
        
        // Если адрес пользователя не найден, используем адрес по умолчанию
        if (!networkInfo || !networkInfo.address) {
            console.warn('Адрес пользователя для TON не найден, используем адрес по умолчанию');
            const defaultNetworkInfo = getAddress('ton');
        ...
        if (networkInfo) {
            ...
            showNetworkError('TON');
        }
    } catch (error) {
        console.error('Ошибка загрузки TON адреса:', error);
        showNetworkError('TON');
    }
}
